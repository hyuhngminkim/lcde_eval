...
//    }
    ...
    string generate_key2(string& key) {
        string result;
        if (key.length() > key_size) {
            result = key.erase(key_size, string::npos);
        } else {
    ...
    //     return std::move(result);
    // }
    // MOD: modify function to prevent overflow & underflow
    string generate_value(uint64_t value) {
        string value_string = to_string(value);
        string result;
        if (value_string.length() > value_size) {
            result = value_string.erase(value_size, string::npos);
        } else {
            result = string(value_size - value_string.length(), '0') + value_string;
        }
